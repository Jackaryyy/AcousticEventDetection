...
#Dataset params
DATASET_PATH = ('dataset/UrbanSound8K/audio/spec/')
...
def loadImageAndTarget(path, doAugmentation=True):
    ...
    label_fan = path.split("\\")[-2]
    label = label_fan.split("/")[-1]
...
print(l.get_all_layers(NET)[0].input_var, targets, lr_dynamic)
...
def showProgress(stat, duration, current, end=batches_per_epoch, update_interval=5, simple_mode=False):
    ...
    progress = ""
    for s in range(update_interval, 100, update_interval):
        if s <= p:
            progress += "="
        else:
            progress += " "        
...
val_accuracy = []
max_acc = -1
lr = LEARNING_RATE[list(LEARNING_RATE.keys())[0]]
...
for epoch in range(EPOCH_START, EPOCHS + 1):
    ...
    try:
        ...
        if LR_DESCENT:
            lr_keys = np.array(list(LEARNING_RATE.keys()) + [EPOCHS], dtype='float32')
            lr_values = np.array(list(LEARNING_RATE.values()) + [list(LEARNING_RATE.values())[-1]], dtype='float32')
            lr_func = interpolate.interp1d(lr_keys, lr_values, kind='linear')
            lr = np.float32(lr_func(max(list(LEARNING_RATE.keys())[0], epoch - 1)))
        else:
        ...
        for image_batch, target_batch in bg.threadedBatchGenerator(getNextImageBatch(split=TRAIN, doAugmentation=True, batchAugmentation=MULTI_LABEL)):            
